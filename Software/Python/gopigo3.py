...
class GoPiGo3(object):
    ...
    LED_WIFI = (
        0x80  # Used to indicate WiFi status. Should not be controlled by the user.
    )
    ...
    def __init__(
        self, addr=8, detect=True, config_file_path="/home/pi/Dexter/gpg3_config.json"
    ):
        """
        ...
        * Optionally disable the detection of the GoPiGo3 hardware. This can be used for debugging
        ...
        if detect:
            try:
                ...
                raise IOError(
                    "No SPI response. GoPiGo3 with address %d not connected." % addr
                )
    ...
    def _check_serial_number_for_16_ticks(
        ...
        try:
            ...
            # List_of_serial_numbers file doesn't exist, assume 6 ticks
            return False
    ...
    def load_robot_constants(self, config_file_path="/home/pi/Dexter/gpg3_config.json"):
        ...
        # Default values for ticks and motor_gear_ratio in case they are not present in the file we load up
        ...
        try:
            with open(config_file_path, "r") as json_file:
                ...
                if (
                    data["wheel-diameter"] > 0
                    and data["wheel-base-width"] > 0
                    and ticks > 0
                    and motor_gear_ratio > 0
                ):
                    self.set_robot_constants(
                        data["wheel-diameter"],
                        data["wheel-base-width"],
                        ticks,
                        motor_gear_ratio,
                    )
            ...
            except:  # Protecting against write errors - just in case
                pass
    ...
    def set_robot_constants(
        self, wheel_diameter, wheel_base_width, ticks, motor_gear_ratio
    ):
        """
        ...
        self.MOTOR_TICKS_PER_DEGREE = (
            self.MOTOR_GEAR_RATIO * self.ENCODER_TICKS_PER_ROTATION
        ) / 360.0
    ...
    def get_manufacturer(self):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.GET_MANUFACTURER,
            0,
            0,
            0,
            0,
    ...
    def get_board(self):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.GET_NAME,
            0,
            0,
            0,
            0,
    ...
    def set_servo(self, servo, us):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.SET_SERVO,
            servo,
            ((us >> 8) & 0xFF),
            (us & 0xFF),
        ]
    ...
    def set_motor_power(self, port, power):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.SET_MOTOR_PWM,
            port,
            int(power),
        ]
    ...
    def set_motor_position(self, port, position):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.SET_MOTOR_POSITION,
            port,
    ...
    def set_motor_dps(self, port, dps):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.SET_MOTOR_DPS,
            int(port),
            ((dps >> 8) & 0xFF),
            (dps & 0xFF),
        ]
    ...
    def set_motor_limits(self, port, power=0, dps=0):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.SET_MOTOR_LIMITS,
            int(port),
            int(power),
            ((dps >> 8) & 0xFF),
            (dps & 0xFF),
    ...
    def get_motor_status(self, port):
        ...
        if port == self.MOTOR_LEFT:
            ...
            raise IOError(
                "get_motor_status error. Must be one motor port at a time. MOTOR_LEFT or MOTOR_RIGHT."
            )
            return
        ...
        if reply[3] == 0xA5:
            ...
            return [
                reply[4],
                power,
                int(encoder / self.MOTOR_TICKS_PER_DEGREE),
                int(dps / self.MOTOR_TICKS_PER_DEGREE),
            ]
    ...
    def offset_motor_encoder(self, port, offset):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.OFFSET_MOTOR_ENCODER,
            int(port),
            ((offset >> 24) & 0xFF),
            ((offset >> 16) & 0xFF),
            ((offset >> 8) & 0xFF),
    ...
    def reset_motor_encoder(self, port):
        ...
        if port & self.MOTOR_LEFT:
            self.offset_motor_encoder(
                self.MOTOR_LEFT, self.get_motor_encoder(self.MOTOR_LEFT)
            )

        if port & self.MOTOR_RIGHT:
            self.offset_motor_encoder(
                self.MOTOR_RIGHT, self.get_motor_encoder(self.MOTOR_RIGHT)
            )
    ...
    def set_grove_pwm_duty(self, pin, duty):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.SET_GROVE_PWM_DUTY,
            pin,
            ((duty_value >> 8) & 0xFF),
            (duty_value & 0xFF),
        ]
    ...
    def set_grove_pwm_frequency(self, port, freq=24000):
        ...
        outArray = [
            self.SPI_Address,
            self.SPI_MESSAGE_TYPE.SET_GROVE_PWM_FREQUENCY,
            port,
            ((freq_value >> 8) & 0xFF),
            (freq_value & 0xFF),
        ]
    ...
    def grove_i2c_transfer(self, port, addr, outArr, inBytes=0):
        ...
        while not Continue:
            try:
                ...
                if time.time() > Timeout:
                    raise IOError(
                        "grove_i2c_transfer error: Timeout trying to start transaction"
                    )
        ...
        if inBytes:
            DelayTime += 1 + inBytes
        DelayTime *= (
            0.000115  # Each I2C byte takes about 115uS at full speed (about 100kbps)
        )
    ...
    def grove_i2c_start(self, port, addr, outArr, inBytes=0):
        ...
        if inBytes > self.GROVE_I2C_LENGTH_LIMIT:
            raise RuntimeError(
                "Read length error. Up to %d bytes can be read in a single transaction."
        ...
        if outBytes > self.GROVE_I2C_LENGTH_LIMIT:
            raise RuntimeError(
                "Write length error. Up to %d bytes can be written in a single transaction."
                % self.GROVE_I2C_LENGTH_LIMIT
            )
        ...
        if self.GroveType[port_index] == self.GROVE_TYPE.IR_DI_REMOTE:
            ...
            if reply[3] == 0xA5:
                if reply[4] == self.GroveType[port_index] and reply[5] == 0:
                    ...
                    if value == 0:
                        raise SensorError(
                            "get_grove_value error: Sensor not responding"
                        )
    ...
    def reset_all(self):
        ...
        self.set_led(
            self.LED_EYE_LEFT
            + self.LED_EYE_RIGHT
            + self.LED_BLINKER_LEFT
            + self.LED_BLINKER_RIGHT,
            0,
            0,
            0,
        )
